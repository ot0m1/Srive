...
const ResultArtists: NextPage<Props> = ({ artist, index }) => {
  ...
  return (
    <p onClick={getTracks} id={artist.id} data-index={index} className="cursor-pointer border-b mb-1 py-1 pl-1 border-slate-100/30 flex items-center rounded hover:bg-slate-200/20">
      <Image src={artistImage()} alt='artist-image' width={46} height={46}/>
      &nbsp;&nbsp;&nbsp;{artist.name}
    </p>
  )
}
